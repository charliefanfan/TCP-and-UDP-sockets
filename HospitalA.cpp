...
cout<<"<Hospital A> has TCP port "<<ntohs(serv.sin_port)<<" and IP address "<<ip<<" for the Phase 1"<<endl;
cout<<"<Hospital A> is now connected to the health center"<<endl;
//discover
char line[50],line1[50],line2[50];  
ifstream file("HospitalA.txt");   //read the text
file>>line>>line1>>line2;      //store the data in the text
int a,b,c;
char which[2]="A";

if(a=(send(sock,line,50,0))>0){
cout<<"<Hospital A> has sent <A1> to the health center"<<endl;}
if(b=(send(sock,line1,50,0))>0){
cout<<"<Hospital A> has sent <A2> to the health center"<<endl;}
if(c=(send(sock,line2,50,0))>0){
cout<<"<Hospital A> has sent <A3> to the health center"<<endl;}
if(a>0&&b>0&&c>0){
send(sock,which,strlen(which),0);
cout<<"Updating the health center is done for <Hospital A>"<<endl;}

close(sock);
cout<<"End of Phase 1 for <Hospital A>"<<endl;
...
clientudp.sin_port = htons(21434);
...
cout<<"<HospitalA> has UDP port " <<DPORT<<" and IP address "<< ip<<" for Phase 2"<<endl;
cout<<"<Student"<<rec<<">"<<" has been admitted to <HospitalA>"<<endl;
t=recvfrom(sock1, rec, 1024, 0, (struct sockaddr *)&peer, &peerlen);
cout<<"<Student"<<rec<<">"<<" has been admitted to <HospitalA>"<<endl;
t=recvfrom(sock1, rec, 1024, 0, (struct sockaddr *)&peer, &peerlen);
cout<<"<Student"<<rec<<">"<<" has been admitted to <HospitalA>"<<endl;
